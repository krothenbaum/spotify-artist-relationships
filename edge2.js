  ...
  $.ajax({
    ...
    data: {
      ...
      success: function (response) {
        ...
        ARTISTS.push({'name': response.artists.items[0].name, 'imports': []})
    ...
    success: function (response) {
      if (degree < 1 && ARTISTIDS.indexOf(this.id) == -1) {
        $(response.artists).each(function () {
          ...
          ARTISTS.push({'name': this.name, 'imports': []})
          index = ARTISTS.findIndex(x => x.name == this.name);
          searchRecommendations(this.id, index, degree+1)
        });
...
function packageHierarchy(classes) {
  ...
  function find(name, data) {
    ...
    if (!node) {
      ...
      if (name.length) {
        console.log(name);
        console.log(name.substring(0, i = name.lastIndexOf(".")));
        console.log(map);
